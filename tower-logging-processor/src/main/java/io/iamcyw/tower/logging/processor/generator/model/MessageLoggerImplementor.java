...
import io.iamcyw.tower.logging.processor.model.LoggerMessageMethod;
...
public class MessageLoggerImplementor extends ImplementationClassModel {
    ...
    private void createLoggerMethod(final LoggerMessageMethod messageMethod, final JClassDef classDef, final JAssignableExpr logger) {
        ...
        if (messageMethod.isAnnotatedWith(Once.class)) {
            ...
            body = baseBody._if(logger.call("isEnabled").arg($v(messageMethod.logLevel()))).block(JBlock.Braces.REQUIRED);
        } else {
            body = baseBody;
        }
